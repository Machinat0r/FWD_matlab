%% Juno bow-shock crossing on 2016-07-17: three days before and after
% This is the Hospodarsky et al. (2017) 15:33 UTC outbound crossing.
% The displayed interval is exactly shock time +/- 3 days.
%
% Science-data rule:
% - JADE was off during this interval. The magnetic panel reads the direct
%   PC Bx/By/Bz fields used by Hospodarsky et al. (2017).
% - XYZ uses the direct MAG PC position fields and is converted from km to
%   R_J using 71492 km/R_J. MAG PC has no direct R field, so that row is --.
    ...
    try
        ...
        if strcmp(event.fieldKind,'JADE_RTP')
            [magTime,magComponents,positionTime,positionXyzKm,positionRadiusRj] = ...
                readJadeContext(event.fieldFiles{1},event.timeRange);
        ...
        elseif strcmp(event.fieldKind,'MAG_PC')
            [magTime,magComponents,positionTime,positionXyzKm] = ...
                readJunoMagPcFiles(event.fieldFiles,event.timeRange);
...
manifestFile = fullfile(outputDir, ...
    'juno_bowshock_20160717_1533_pm3days_manifest.csv');
...
function events = defineEvents(root)
    shockTime = datenum(2016,7,17,15,33,0);
    dayOfYear = 196:202;
    waveFiles = arrayfun(@(d) fullfile(root,'Waves_data', ...
        '2016186_ORBIT_00',sprintf('WAV_2016%03dT000000_E_V02.CSV',d)), ...
        dayOfYear,'UniformOutput',false);
    magFiles = arrayfun(@(d) fullfile(root,'MAG','2016','JUPITER','PC', ...
        'PERI-00',sprintf('fgm_jno_l3_2016%03dpc_r1s_v01.sts',d)), ...
        dayOfYear,'UniformOutput',false);

    events = makeEvent( ...
        'orbit0_20160717_pm3days',shockTime+[-3 3],shockTime, ...
        1440,30,'out', ...
        'Hospodarsky et al. (2017)', ...
        '10.1002/2017GL073177', ...
        waveFiles,'MAG_PC',magFiles, ...
        'juno_bowshock_20160717_1533_pm3days_hospodarsky2017');
end

function event = makeEvent(id,timeRange,shockTime,tickStepMinutes, ...
        waveBinSeconds,direction,article,doi,waveFiles,fieldKind, ...
        fieldFiles,outputTag)
    event = struct('id',id,'timeRange',timeRange,'shockTime',shockTime, ...
        ...
        'fieldFiles',{fieldFiles},'outputTag',outputTag, ...
        'maxPositionOffsetMinutes',10);
    for iFile = 1:numel(waveFiles)
        assert(isfile(waveFiles{iFile}),'Missing Waves file: %s',waveFiles{iFile});
    end
    for iFile = 1:numel(fieldFiles)
        assert(isfile(fieldFiles{iFile}),'Missing field file: %s',fieldFiles{iFile});
    end
end
...
function [timeNum,magneticField,positionTime,positionKm] = ...
        readJunoMagPcFiles(files,timeRange)
    timeParts = cell(numel(files),1);
    fieldParts = cell(numel(files),1);
    positionParts = cell(numel(files),1);
    for iFile = 1:numel(files)
        fprintf('  MAG PC %d/%d: %s\n',iFile,numel(files),files{iFile});
        [timeParts{iFile},fieldParts{iFile},~,positionParts{iFile}] = ...
            readJunoMagPc(files{iFile},timeRange);
    end
    timeNum = vertcat(timeParts{:});
    magneticField = vertcat(fieldParts{:});
    positionKm = vertcat(positionParts{:});
    [timeNum,order] = sort(timeNum);
    magneticField = magneticField(order,:);
    positionKm = positionKm(order,:);
    [timeNum,keep] = unique(timeNum,'stable');
    magneticField = magneticField(keep,:);
    positionKm = positionKm(keep,:);
    positionTime = timeNum;
    assert(any(any(isfinite(magneticField))), ...
        'No direct MAG PC records in the requested interval.');
end
...
function addTimeAndPositionLabels(ax,timeRange,tickValues,positionRj)
    ...
    for iTick = 1:numel(tickValues)
        ...
        text(ax,xNorm(iTick),-0.036,datestr(tickValues(iTick),timeFormat), ...
