...
function pad=c_rapid_get_pad_bf2004(varargin)
% c_rapid_get_pad_bf2004 calculate the electron pitch angle distribution of RAPID before year 2004
%
...
%
%Output parameter: electron PSD in the structure format
%---------------------------------------
%Example: 
%pad=c_rapid_get_pad_bf2004(C1_CP_RAP_EPADEX2, C1_CP_RAP_EPITCH, 1);
%---------------------------------------

%----created by Huishan Fu at IRFU----
...
E_channel=args{3};
    ...
    for kk=1:length(ttt)
        ...
        for ii=1:16
            for jj=1:9
                ind=find(angtemp>=Pitang_each(ii,jj)); 
                if numel(ind)>0; index=ind(1)-1; end
    ...
    pad=struct('Time',ttt,'Pitang',Pitang,'RapidFlux',Flux,'Pitang_unit','Pitch angle [deg]');
