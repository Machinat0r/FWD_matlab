function h=hs_c_pl_sc_conf_xyz(time,coord_sys,flag,spacecraft)
...
% $Id: c_pl_sc_conf_xyz.m,v 1.13 2010/05/20 15:00:47 andris Exp $
...
if strcmp(action,'initialize'),
    ...
    switch plot_type
        ...
        case 'publish'
            ...
            h(1)=subplot(2,2,1);axis([-19.99 0.00 -9.99 9.99]);
            h(2)=subplot(2,2,2);axis([-19.99 0.00 -9.99 9.99]);
            h(3)=subplot(2,2,3);axis([-50 50 -50 50]);
            h(4)=subplot(2,2,4);axis([-50 50 -50 50]);
        case 'label'
            set(figNumber,'Position',[10 10 700 700]);clear h;
    ...
    axis off;
    ht=irf_pl_info(['hs_c_pl_sc_conf_xyz() ' datestr(now)],gca,[0,1 ]); set(ht,'interpreter','none');
...
elseif strcmp(action,'plot'),
    ...
    switch plot_type
        ...
        case 'publish'
            axes(h(1));   axis(h(1), 'equal');
            c_eval('plot(XRe?(2),XRe?(4),cluster_marker{?},''LineWidth'',1.5);hold on;',sc_list);
            xlabel(['X [R_E] ' coord_label]);ylabel(['Z [R_E] '  coord_label]);
            grid on; box on; axis([-20 0 -6 6]);
            set(gca,'xdir','reverse')
            
            axes(h(2));   axis(h(2), 'equal');
            c_eval('plot(XRe?(2),XRe?(3),cluster_marker{?},''LineWidth'',1.5);hold on;',sc_list);
            xlabel(['X [R_E] ' coord_label]);ylabel(['Y [R_E] ' coord_label]);
            grid on; box on; axis([-20 0 -6 6]);
            set(gca,'xdir','reverse')
            ...
            set(gca,'xdir','reverse')
            grid on; axis([-drref drref -drref drref]);  %axis([-80 40 -90 30]);
            if drref>1000, REform='%6.1f';
            elseif drref<100, REform='%6.3f';
            else REform='%6.2f';
            end 
            
            axes(h(4)); cla; axes(h(4)); ax1=gca;
            c_eval('plot(x?(2),x?(3),cluster_marker{?},''LineWidth'',1.5);hold on;',sc_list);
        ...
        case 'label'
            ...
            c_eval('plot(XRe?(2),XRe?(4),cluster_marker{?},''LineWidth'',1.5);hold on;',sc_list);
            xlabel(['X [R_E] ' coord_label]);ylabel(['Z [R_E] '  coord_label]);
            grid on; box on; axis([-16 -12 -4 0]);
            set(gca,'xdir','reverse')
            
            axes(h(2));   axis(h(2), 'equal');
            c_eval('plot(XRe?(2),XRe?(3),cluster_marker{?},''LineWidth'',1.5);hold on;',sc_list);
            xlabel(['X [R_E] ' coord_label]);ylabel(['Y [R_E] ' coord_label]);
            grid on; box on; axis([-16 -12 1 5]);
            set(gca,'xdir','reverse')
            set(gca,'ydir','reverse')
            ...
            xlabel(['{\Delta}X [km] ' coord_label]);ylabel(['{\Delta}Z [km] ' coord_label]);
            set(gca,'xdir','reverse')
            grid on; axis([-25 25 -30 20]);  %axis([-drref drref -drref drref]);
            if drref>1000, REform='%6.1f';
            elseif drref<100, REform='%6.3f';
            else REform='%6.2f';
            end 
            ...
            c_eval('plot(x?(2),x?(3),cluster_marker{?},''LineWidth'',1.5);hold on;',sc_list);
            xlabel(['{\Delta}X [km] ' coord_label]);ylabel(['{\Delta}Y [km] ' coord_label]);
            grid on;  axis([-25 25 -25 25]);  %axis([-drref drref -drref drref]);
            set(gca,'xdir','reverse')
            set(gca,'ydir','reverse')
