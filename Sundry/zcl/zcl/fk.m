...
switch rem(length(YY_PSD1),5)
    case 0 
        YY_PSD3=YY_PSD1;
    case 1
        YY_PSD3=YY_PSD1(1:end-1);
    case 2
        YY_PSD3=YY_PSD1(1:end-2);
    case 3
        YY_PSD3=YY_PSD1(1:end-3);
    case 4
        YY_PSD3=YY_PSD1(1:end-4); 
end

YY_PSD2 = (YY_PSD3(1:5:end,:)+YY_PSD3(2:5:end,:)+YY_PSD3(3:5:end,:)+ ...
    YY_PSD3(4:5:end,:)+YY_PSD3(5:5:end,:))/5;
temp_id = [];
[row,~] = size(YY_PSD2);
for i = 1:row
    temp_diff = diff(YY_PSD2(i,18:51));
    flag = 0;
    for j = 1:length(temp_diff)-5
        if temp_diff(j) > 0 && temp_diff(j+1) > 0 && temp_diff(j+2) > 0 && ...
                temp_diff(j+3) < 0 && temp_diff(j+4) < 0 && temp_diff(j+5) < 0
%             temp_max(i) = YY_PSD1(i,j+3);  
            if flag == 0
                temp_id(end+1)= j+3;
            end
            flag = 1;
...
E_max = Energy(max(temp_id)+18)
E_min = Energy(min(temp_id)+18)
