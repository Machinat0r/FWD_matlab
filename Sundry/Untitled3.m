clear,clc
close all
load('C:\Matlab\bin\新建文件夹\fwd\Sundry\GOLD_L2_TDISK_2020_230_v03_r01_c01.mat');
...
for m=1:46
    for n=1:52
        ...
        for i = 2:length(index)
            ...
            if isempty(strfind(tmpTime2,'*')) && iso2epoch(tmpTime2(1:20)) - TimeFlag1 > 3600*5
                time1{:,m,n,1}=tmpTime1;
                time2{:,m,n,1}=tmpTime2;
                T{m,n,:} = tdisk(m,n,index([1,i]));
                flag = 1;
...
for a=1:46
    for b=1:52
        ...
        Time1{a,b} = iso2epoch(time11) - iso2epoch('2020-10-26T00:00:00Z');
        Time2{a,b} = iso2epoch(time22) - iso2epoch('2020-10-26T00:00:00Z');
        else
        Time1{a,b} = 0;
        Time2{a,b} = 0;
        end
...
fun = @(X,xdata)2*X(1).*cos(Ravote.*dt/2).*cos(Ravote.*t2+Ravote.*dt/2-4.*xdata-X(2))...
    +2.*X(3).*cos(Ravote*dt/2).*cos(Ravote.*t2+Ravote.*dt/2-3.*xdata-X(4));
