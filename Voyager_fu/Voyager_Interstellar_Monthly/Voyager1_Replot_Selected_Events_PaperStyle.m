...
%Voyager1_Replot_Selected_Events_PaperStyle Replot selected Voyager events.
    ...
    if strcmp(opts.CRSDisplay, 'lecp_p1_pitch_angle')
        if strcmp(opts.PitchAngleMethod, 'florinski2008')
            pitchAngleFiles{ii} = fullfile(opts.PitchAngleDataFolder, ...
                sprintf(['V%d_%s_%s_%s_LECP_P1_Florinski2008_', ...
                'pitch_angle_1d.csv'], opts.Spacecraft, catalog.EventID{ii}, ...
                datestr(catalog.StartUTC(ii), 'yyyymmdd'), ...
                datestr(catalog.EndUTCInclusive(ii), 'yyyymmdd'))); %#ok<DATST>
        else
            pitchAngleFiles{ii} = fullfile(opts.PitchAngleDataFolder, ...
                sprintf(['V%d_%s_%s_%s_LECP_P1_fullVector_', ...
                'pitch_angle_native.csv'], opts.Spacecraft, ...
                catalog.EventID{ii}, ...
                datestr(catalog.StartUTC(ii), 'yyyymmdd'), ...
                datestr(catalog.EndUTCInclusive(ii), 'yyyymmdd'))); %#ok<DATST>
        end
