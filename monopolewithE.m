...
OutputDir = [ParentDir,'CaseforE/'];
filepath = [ParentDir, 'CaseforE/Case06.txt'];
TimeCell = readcell(filepath);
for Cellidx = 1:length(TimeCell)
TempCell = TimeCell{Cellidx};
if isempty(regexp(TempCell, '[0-9]{8}','match'))
    continue
else
TimeTint = regexp(TempCell, '[0-9]{8}\s.+', 'match');
TT = datenum(TimeTint,'yyyymmdd HH:MM:SS.FFF');
TT = [datestr(TT - 5/86400, 'yyyy-mm-ddTHH:MM:SS.FFFZ'), '/'...
    , datestr(TT + 5/86400, 'yyyy-mm-ddTHH:MM:SS.FFFZ')];
...
FigDir = [OutputDir,'meanFig/'];
mkdir(FigDir);mkdir(tempDir);
...
%% Etotal
h(i)=irf_subplot(n,1,-i);
irf_plot([Et1(:,1) Et1(:,5)], 'color','k', 'Linewidth',0.75); hold on;
irf_plot([Et2(:,1) Et2(:,5)], 'color','r', 'Linewidth',0.75); hold on;
irf_plot([Et3(:,1) Et3(:,5)], 'color','g', 'Linewidth',0.75); hold on;
irf_plot([Et4(:,1) Et4(:,5)], 'color','b', 'Linewidth',0.75); hold on;
%irf_plot([B1(:,1) B1], 'color','k', 'Linewidth',0.75); hold on;
c_eval("irf_plot([E?(:,1) 0*E?(:,5)],'k--', 'Linewidth',0.75);",ic); hold off;
grid off;
set(gca,'Ylim',[min([min(Et1(:,5)) min(Et2(:,5)) min(Et3(:,5)) min(Et4(:,5))])-5 ...
    max([max(Et1(:,5)) max(Et2(:,5)) max(Et3(:,5)) max(Et4(:,5))])+5]);
% c_eval("set(gca,'Ylim',[min([min(B?_gsm(:,2))])-100 max([max(B?_gsm(:,2))])+100]);",ic);
% set(gca,'Ylim',[-40 60], 'ytick',[-40:20:60],'fontsize',9);
pos1=get(gca,'pos');
set(gca,'ColorOrder',[[0 0 0];[1 0 0];[0 1 0];[0 0 1]]);
% irf_legend(gca,{'MMS1','MMS2','MMS3','MMS4'},[0.97 0.92]);
ylabel('|E| [nT]','fontsize',12);
% irf_legend(gca,{'B_N'},[pos2(1)+1.15*pos2(3),pos2(2)]);
% irf_legend(gca,'a',[0.99 0.98],'color','k','fontsize',12)
i=i+1;
%% Ex
h(i)=irf_subplot(n,1,-i);
irf_plot([E1(:,1) E1(:,2)], 'color','k', 'Linewidth',0.75); hold on;
irf_plot([E2(:,1) E2(:,2)], 'color','r', 'Linewidth',0.75); hold on;
irf_plot([E3(:,1) E3(:,2)], 'color','g', 'Linewidth',0.75); hold on;
irf_plot([E4(:,1) E4(:,2)], 'color','b', 'Linewidth',0.75); hold on;
%irf_plot([B1(:,1) B1], 'color','k', 'Linewidth',0.75); hold on;
c_eval("irf_plot([E?(:,1) 0*E?(:,2)],'k--', 'Linewidth',0.75);",ic); hold off;
grid off;
set(gca,'Ylim',[min([min(E1(:,2)) min(E2(:,2)) min(E3(:,2)) min(E4(:,2))])-5 ...
    max([max(E1(:,2)) max(E2(:,2)) max(E3(:,2)) max(E4(:,2))])+5]);
% c_eval("set(gca,'Ylim',[min([min(B?_gsm(:,2))])-100 max([max(B?_gsm(:,2))])+100]);",ic);
% set(gca,'Ylim',[-40 60], 'ytick',[-40:20:60],'fontsize',9);
pos1=get(gca,'pos');
set(gca,'ColorOrder',[[0 0 0];[1 0 0];[0 1 0];[0 0 1]]);
% irf_legend(gca,{'MMS1','MMS2','MMS3','MMS4'},[0.97 0.92]);
ylabel('Ex [nT]','fontsize',12);
% irf_legend(gca,{'B_N'},[pos2(1)+1.15*pos2(3),pos2(2)]);
% irf_legend(gca,'a',[0.99 0.98],'color','k','fontsize',12)
i=i+1;
%% Ey
h(i)=irf_subplot(n,1,-i);
irf_plot([E1(:,1) E1(:,3)], 'color','k', 'Linewidth',0.75); hold on;
irf_plot([E2(:,1) E2(:,3)], 'color','r', 'Linewidth',0.75); hold on;
irf_plot([E3(:,1) E3(:,3)], 'color','g', 'Linewidth',0.75); hold on;
irf_plot([E4(:,1) E4(:,3)], 'color','b', 'Linewidth',0.75); hold on;
%irf_plot([B1(:,1) B1], 'color','k', 'Linewidth',0.75); hold on;
c_eval("irf_plot([E?(:,1) 0*E?(:,3)],'k--', 'Linewidth',0.75);",ic); hold off;
grid off;
set(gca,'Ylim',[min([min(E1(:,3)) min(E2(:,3)) min(E3(:,3)) min(E4(:,3))])-5 ...
    max([max(E1(:,3)) max(E2(:,3)) max(E3(:,3)) max(E4(:,3))])+5]);
% c_eval("set(gca,'Ylim',[min([min(B?_gsm(:,2))])-100 max([max(B?_gsm(:,2))])+100]);",ic);
% set(gca,'Ylim',[-40 60], 'ytick',[-40:20:60],'fontsize',9);
pos1=get(gca,'pos');
set(gca,'ColorOrder',[[0 0 0];[1 0 0];[0 1 0];[0 0 1]]);
% irf_legend(gca,{'MMS1','MMS2','MMS3','MMS4'},[0.97 0.92]);
ylabel('Ey [nT]','fontsize',12);
% irf_legend(gca,{'B_N'},[pos2(1)+1.15*pos2(3),pos2(2)]);
% irf_legend(gca,'a',[0.99 0.98],'color','k','fontsize',12)
i=i+1;
%% Ez
h(i)=irf_subplot(n,1,-i);
irf_plot([E1(:,1) E1(:,4)], 'color','k', 'Linewidth',0.75); hold on;
irf_plot([E2(:,1) E2(:,4)], 'color','r', 'Linewidth',0.75); hold on;
irf_plot([E3(:,1) E3(:,4)], 'color','g', 'Linewidth',0.75); hold on;
irf_plot([E4(:,1) E4(:,4)], 'color','b', 'Linewidth',0.75); hold on;
%irf_plot([B1(:,1) B1], 'color','k', 'Linewidth',0.75); hold on;
c_eval("irf_plot([E?(:,1) 0*E?(:,4)],'k--', 'Linewidth',0.75);",ic); hold off;
grid off;
set(gca,'Ylim',[min([min(E1(:,4)) min(E2(:,4)) min(E3(:,4)) min(E4(:,4))])-5 ...
    max([max(E1(:,4)) max(E2(:,4)) max(E3(:,4)) max(E4(:,4))])+5]);
% c_eval("set(gca,'Ylim',[min([min(B?_gsm(:,2))])-100 max([max(B?_gsm(:,2))])+100]);",ic);
% set(gca,'Ylim',[-40 60], 'ytick',[-40:20:60],'fontsize',9);
pos1=get(gca,'pos');
set(gca,'ColorOrder',[[0 0 0];[1 0 0];[0 1 0];[0 0 1]]);
% irf_legend(gca,{'MMS1','MMS2','MMS3','MMS4'},[0.97 0.92]);
ylabel('Ez [nT]','fontsize',12);
% irf_legend(gca,{'B_N'},[pos2(1)+1.15*pos2(3),pos2(2)]);
% irf_legend(gca,'a',[0.99 0.98],'color','k','fontsize',12)
i=i+1;
%% η
...
colormap(jet)
figname = [FigDir,datestr(datenum(1970,1,1,0,0,0)+B1(1)/86400,'yyyymmddTHHMMSS')];    
...
set(gcf,'render','painters');
set(gcf,'paperpositionmode','auto')
figname = [FigDir,datestr(datenum(1970,1,1,0,0,0)+B1(tempidx_B1,1)/86400,'yyyymmddTHHMMSS'),'-Configuration'];
savefig([figname '.fig']);
clf
end
end
