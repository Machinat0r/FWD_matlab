...
inpath = '/Users/fwd/Documents/MATLAB/Code/XIE_HS/pdrk/pdrk_master_v181027/pdrk-master/ECW/input/pdrk.in';
%% Load in data
units = irf_units;
...
hold on;
plot(h(1),energypad1,FbiMaxwell1); hold on
...
plot(h(2),energypad1,psd2,'k.-','LineWidth',1);
hold on;
plot(h(2),energypad1,FbiMaxwell2);
ylabel(h(2),'f_e (s^3 km^{-6})');
xlabel(h(2),'E (eV)');
set(h(2),'yscale','log');
set(h(2),'xscale','log');
title(h(2),strcat('F_{\perp}'));

irf_zoom(h(2),'y',yrange);
irf_zoom(h(2),'x',[30 15000]);
%% Maxwell function
