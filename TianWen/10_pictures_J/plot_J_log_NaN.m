...
load(['E:\Martian\programs\2_Maven_Download\10_pictures_J\J_log_NaN\Ja' ...
...
load(['E:\Martian\programs\2_Maven_Download\10_pictures_J\B\Ba' ...
    , num2str(gridNuma), 'e', num2str(gridNume), 'h', num2str(gridNumh), '.mat']);
load(['E:\Martian\programs\2_Maven_Download\10_pictures_J\B\Ba' ...
    , num2str(gridNuma), 'e', num2str(gridNume), 'h', num2str(gridNumh), '_day', '.mat']);
load(['E:\Martian\programs\2_Maven_Download\10_pictures_J\B\Ba' ...
...
for k = 1 : 3
    ...
    for slice = 1 : n_slice - 1
        ...
        eval(['b = surf(aGrid(:, :, 1), eGrid(:, :, 1), z, logJ', coor, cell2mat(LA(k)), '(:,:,slice));'])
        % b=surf(aGrid(:, :, 1), eGrid(:, :, 1), z, logJa(:, :, slice));
        ...
        yl=ylabel('Latitude (°)','rotation',-39);
