...
inputFile = [ParentDir, 'T_BRsphere_za_year_15_23.mat'];
load(inputFile);
...
% parpool(12)
disp("average B_'all' in grid...")
parfor_progress(numel(aGrid));
...
% 现在更改函数，将矩阵缩小再进行尝试，利用空矩阵删除矩阵的最后一行（列、页）。
% 而在计算完毕梯度后，恢复矩阵的大小。
BaGrid(gridNuma, :, :) = []; BaGrid(:, gridNume, :) = []; BaGrid(:, :, gridNumh) = [];
BeGrid(gridNuma, :, :) = []; BeGrid(:, gridNume, :) = []; BeGrid(:, :, gridNumh) = [];
BhGrid(gridNuma, :, :) = []; BhGrid(:, gridNume, :) = []; BhGrid(:, :, gridNumh) = [];
...
%% calculate J
disp("calculating J_'all'...")
c_eval('grid? = movmean(grid?, 2);', coor);
c_eval('grid? = grid?(2:end);', coor);
[aGrid, eGrid, hGrid] = meshgrid(grida, gride, gridh); 
c_eval('?Grid = permute(?Grid, [2, 1, 3]);',coor); 
% averaging grid
...
parfor_progress(0);
BaGrid_day(gridNuma, :, :) = []; BaGrid_day(:, gridNume, :) = []; BaGrid_day(:, :, gridNumh) = [];
BeGrid_day(gridNuma, :, :) = []; BeGrid_day(:, gridNume, :) = []; BeGrid_day(:, :, gridNumh) = [];
BhGrid_day(gridNuma, :, :) = []; BhGrid_day(:, gridNume, :) = []; BhGrid_day(:, :, gridNumh) = [];
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% input data (sph)
inputFile = [ParentDir, 'T_BRsphere_za_year_15_23_nig.mat'];
load(inputFile);
...
c_eval('?Grid = permute(?Grid, [2, 1, 3]);',coor); 
% % aGrid = permute(aGrid, [c, 1, 3]);交换1、2维度
...
parfor_progress(0);
BaGrid_nig(gridNuma, :, :) = []; BaGrid_nig(:, gridNume, :) = []; BaGrid_nig(:, :, gridNumh) = [];
BeGrid_nig(gridNuma, :, :) = []; BeGrid_nig(:, gridNume, :) = []; BeGrid_nig(:, :, gridNumh) = [];
BhGrid_nig(gridNuma, :, :) = []; BhGrid_nig(:, gridNume, :) = []; BhGrid_nig(:, :, gridNumh) = [];
