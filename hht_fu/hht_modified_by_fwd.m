function [varargout] = hht_modified_by_fwd(IMF, varargin)
...
narginchk(1,9);
nargoutchk(0,6);
[IMFd,fs,T,TD,F,FRange,FResol,MinThres,Method,FreqLoc,isTT,isNF,nchannels] = parseAndValidateInputs(IMF, varargin{:});
isInMATLAB = isempty(coder.target);

% store instantaneous frequency and energy
freqIdx = zeros(nchannels, length(T));
insf = zeros(length(T), nchannels);
inse = zeros(length(T), nchannels);

for i = 1:nchannels
    switch Method
        % for future extension
        case 'HT'
            sig = signalwavelet.internal.analyticSignal(IMFd(:,i));
            energy = abs(sig).^2;
...
timeIdx = repmat(1:length(T),nchannels,1);
...
if(nargout==0)
    ...
    signalwavelet.internal.guis.plot.hhtPlot(insf, inse, Time, FRange, MinThres, FreqLoc, isNF);
end
...
if nargout > 3
    if isTT
        insfNames = cell(1,nchannels);
        coder.unroll();
        for i = 1:nchannels
            insfNames{i} = coder.const(['insf' sprintf('%d',i)]);
        end
        varargout{4} = array2timetable(insf,'RowTimes',Time,'variableNames',insfNames);
    else
        varargout{4} = insf;
    end
end
...
if nargout > 4
    if isTT
        inseNames = cell(1,nchannels);
        coder.unroll();
        for i = 1:nchannels
            inseNames{i} = coder.const(['inse' sprintf('%d',i)]);
        end
        varargout{5} = array2timetable(inse,'RowTimes',Time,'variableNames',inseNames);
    else
        varargout{5} = inse;
    end
end
if nargout > 5
    varargout{6} = phaseAngle;
end

end

%--------------------------------------------------------------------------
function [IMFd,fs,T,TD,F,FRange,FResol,MinThres,Method,FreqLoc,isTT,isNF,nchannels] = parseAndValidateInputs(IMF, varargin)
% input type checking
isInMATLAB = isempty(coder.target);
...
% handle timetable
if(isTT)
    coder.internal.errorIf(size(IMF,1)<2,'shared_signalwavelet:hht:general:notEnoughRows','IMF',1);
    ...
    if(~isNF)
        coder.internal.errorIf(abs((T(2)-T(1))-1/fs)>eps,'shared_signalwavelet:hht:general:notMatchedFreqTimetable','IMF');
    else
        fs = 1/(T(2)-T(1));
        isNF = false;
    end
...
if ~isempty(args) % atleast one name value pair is present, so parse the inputs
    if(isInMATLAB)
        ...
        poptions = struct( ...
                      ...
                      'StructExpand',false, ...
                      'IgnoreNulls',false);
        pstruct      =  coder.internal.parseParameterInputs(parms,poptions,args{:});
