...
% c_eval('fpiFile? = dataobj(''D:\MALAB\mms_db\data\mms?\fpi\brst\l2\des-dist\2015\12\18\mms1_fpi_brst_l2_des-dist_20151218000914_v2.1.0.cdf'');',ic);
% cd D:\MATLAB\mms_db
mms.db_init('local_file_db','D:\matlab2015a\MMS\')
...
%% 
tint1 = irf_time('2017-08-23T15:38:20Z','utc>epochTT');
[paddist1,thetapad1,energypad1,tintpad1] = mms.get_pitchangledist(diste,phi,theta,stepTable,energy0,energy1,Bxyz,tint1); 
...
%% 
tint2 = irf_time('2017-08-23T15:39:15Z','utc>epochTT');
[paddist2,thetapad2,energypad2,tintpad2] = mms.get_pitchangledist(diste,phi,theta,stepTable,energy0,energy1,Bxyz,tint2); 

paddist2 = paddist2*1e30; %convert to commonly used s^3 km^-6

ymin = 10^-4;
ymax = ceil(max(max(log10(paddist2))));
yrange = [ymin 10^ymax];

jetcolor = colormap('jet');
lll = length(jetcolor(:,1));
vcolors = floor(lll/length(energypad2));
vcolors = [1:length(energypad2)]*vcolors;
    
c_eval('plot(h(2),thetapad2,squeeze(paddist2(?,:)),''color'',jetcolor(vcolors(?),:));',1);
hold(h(2),'on');
c_eval('plot(h(2),thetapad2,squeeze(paddist2(?,:)),''color'',jetcolor(vcolors(?),:));',[2:32]);
hold(h(2),'off')
...
set(h(2),'xtick',[]);
tintutc = tintpad2.utc;
title(h(2),strcat(tintutc(12:23),'UT'));
for i=1:length(energypad2)
    a=roundn(energypad2(1,:)/1000,-3);
c_eval('irf_legend(h(2),''? keV'',[1.01 1-i*0.03],''color'',jetcolor(vcolors(i),:));',a(i));
end
%% 
% set(h(2:6),'ytick',[]);
% set(h(2),'ytick',[]);
set(h(1:2),'xtick',[0 90 180]);
set(gcf,'color','w');
