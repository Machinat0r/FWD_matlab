...
Units=irf_units;
me=Units.me;
tint=irf.tint('2019-07-15T15:24:00.00Z/2019-07-15T15:25:00.00Z');
%
...
c_eval('fpiFilee1 = dataobj(''D:\matlab2019a\MMS\mms1\fpi\brst\l2\des-moms\2019\07\15\mms1_fpi_brst_l2_des-moms_20190715152143_v3.3.0.cdf'');',ic);
...
c_eval('fpiFilee2 = dataobj(''D:\matlab2019a\MMS\mms1\fpi\brst\l2\des-moms\2019\07\15\mms1_fpi_brst_l2_des-moms_20190715152143_v3.3.0.cdf'');',ic);
...
set(gca,'Ylim',[-4 10], 'ytick',[-5 0 5 10 15 20 25]);
pos1=get(gca,'pos');
...
%set(gca,'Ylim',[fix(min([min(Ne1(:,2)) min(Ni1(:,2))]))-2 fix(max([max(Ne1(:,2)) max(Ni1(:,2))]))+2]);
set(gca,'Ylim',[0.2 0.55], 'ytick',[0 0.2 0.4 0.6 0.8 1 1.2 1.6]);
% pos1=get(h(1),'pos');
 set(gca,'ColorOrder',[[0 0 1];[0 1 0]]);
 irf_legend(gca,{'Ne','Ni'},[0.1 0.12]);
% irf_legend(gca,{'B_N'},[pos2(1)+1.15*pos2(3),pos2(2)]);
...
set(gca,'Ylim',[-5 5], 'ytick',[-15 -10 -3 0 3 10 15]);
...
%% Eblmn
% h(i)=irf_subplot(n,1,-i);
% irf_plot([Elmn1(:,1) Elmn1(:,2)], 'color','b', 'Linewidth',0.75); hold on;
% irf_plot([Elmn1(:,1) Elmn1(:,3)], 'color','g', 'Linewidth',0.75); hold on;
% irf_plot([Elmn1(:,1) Elmn1(:,4)], 'color','r', 'Linewidth',0.75); hold on;
% irf_plot([Elmn1(:,1) Elmn1(:,2)*0],'k--', 'Linewidth',0.75); hold off;
% grid off;
% ylabel('Ebf [mV/m]','fontsize',10)
% set(gca,'Ylim',[-0.5 0.5], 'ytick',[-0.5 0 1 2  ]);
% % irf_legend(gca,'c',[0.99 0.98],'color','k','fontsize',12);
% %set(gca,'Ylim',[fix(min([min(E1(:,2)) min(E1(:,3)) min(E1(:,4))])/10)*10-10 fix(max(Et1(:,2))/10)*10+10]);
% set(gca,'ColorOrder',[[0 0 1];[0 1 0];[1 0 0];[0 0 0]]);
...
% h(i)=irf_subplot(n,1,-i);
% colormap(h(i),jet)
% [a9,b9]=irf_spectrogram(h(i),specE,'log');
