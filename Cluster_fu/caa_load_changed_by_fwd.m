function out=caa_load_changed_by_fwd(varargin)
...
shouldReadAllData   = false;  % default load everything
shouldLoadFromFile  = true;  % if false, do not load object from file if it exists in memory
useExactNameMatch   = true; % default is to filter not according to exact match
shouldFilterNames = any(nargin);   % if there is input, use it to filter names, otherwise load all
%% NameTag
Tsta = varargin{2};Tend = varargin{3};
digitFlag1 = isstrprop(Tsta,'digit');
Tsta = Tsta(digitFlag1);
digitFlag2 = isstrprop(Tend,'digit');
Tend = Tend(digitFlag2);
NameTag = [Tsta(1:8) '_' Tsta(9:14) '_' Tend(1:8) '_' Tend(9:14)];
...
% 					evalin('caller',[datasetName '=dataobj(''' caaDataDirectory dirs(j).name filesep '*.cdf'',''tint'',caa_load_tint_temp);']);
                    evalin('caller',[datasetName '=dataobj(''' caaDataDirectory dirs(j).name filesep datasetName '__' NameTag '*.cdf'');']);
