...
% % % % % % TT = '2017-06-11T01:59:25.000/2017-06-11T01:59:55.000';
% % % % % % TT = '2017-06-15T02:16:55.000/2017-06-15T02:17:25.000';
% % % % % % TT = '2018-07-23T11:36:20.000/2018-07-23T11:37:50.000';
% % % TT = '2018-09-16T21:19:50.000/2018-09-16T21:20:07.000';
...
% TT = '2017-07-23T19:49:30.000Z/2017-07-23T19:49:40.000Z';
% % % TT = '2018-12-27T21:50:00.000Z/2018-12-27T22:10:00.000Z';
% % % TT = '2021-09-08T09:31:40.000Z/2021-09-08T09:32:00.000Z';
TT = '2021-07-22T11:18:54.000Z/2021-07-22T11:20:53.000Z';
% TT ='2022-08-19T03:52:17.000Z/2022-08-19T03:52:22.000Z';
 % % % TT = '2021-07-14T17:36:45.000Z/2021-07-14T17:36:52.300Z';
 % TT = '2020-07-24T09:42:07.000Z/2020-07-24T09:42:15.000Z';
 % TT = '2019-09-24T02:16:55.000Z/2019-09-24T02:17:20.000Z';
...
% c_eval('JdotEplus? = [Eplus?(:,1) irf_dot(J_B(:,2:4),Eplus?(:,2:4))];',ic);
c_eval('Eplus? = irf_resamp(E?,J?);',ic);
...
c_eval('desmoms1 = mms.get_filepath(''mms?_fpi_brst_l2_des-moms'',tint);',ic);
c_eval(['fpiFilee1 = dataobj(','''',desmoms1,'''',');'],ic);
c_eval('energy_low1 = get_variable(fpiFilee1,''mms?_des_pitchangdist_lowen_brst'');',ic);
c_eval('energy_mid1 = get_variable(fpiFilee1,''mms?_des_pitchangdist_miden_brst'');',ic);
c_eval('energy_high1 = get_variable(fpiFilee1,''mms?_des_pitchangdist_highen_brst'');',ic);
...
%% Smooth
% % % dspan = 10;
% % % Ve_smooth = [smooth(gsmVe1(:,1),dspan),smooth(gsmVe1(:,2),dspan),...
...
% % % 
% % % Ne_smooth = [smooth(Ne1(:,1),dspan),smooth(Ne1(:,2),dspan)];
% % % Vet_smooth = [smooth(Vet1(:,1),dspan),smooth(Vet1(:,2),dspan)];

% % % c_eval('dte = mean(diff(Te?(:,1)));', ic);
% % % c_eval('dti = mean(diff(Ti?(:,1)));', ic);
% % % % c_eval('tspane = dspan*round(dfB?/meanf);', i);
% % % c_eval('Ne? = [smooth(Ne?(:,1), dspan) smooth(Ne?(:,2), dspan)];', ic);
% % % c_eval('Te? = [smooth(Te?(:,1), dspan) smooth(Te?(:,2), dspan)];', ic);
% % % c_eval('Te_para? = [smooth(Te_para?(:,1), dspan) smooth(Te_para?(:,2), dspan)];', ic);
% % % c_eval('Te_perp? = [smooth(Te_perp?(:,1), dspan) smooth(Te_perp?(:,2), dspan)];', ic);
% % % c_eval('Ni? = [smooth(Ni?(:,1), dspan) smooth(Ni?(:,2), dspan)];', ic);
% % % c_eval('Ti? = [smooth(Ti?(:,1), dspan) smooth(Ti?(:,2), dspan)];', ic);
% % % c_eval('gsmVi? = [smooth(gsmVi?(:,1), dspan) smooth(gsmVi?(:,2), dspan) smooth(gsmVi?(:,3), dspan) smooth(gsmVi?(:,4), dspan)];', ic);
%% lmn
% irf_minvar_gui(B1);
...
% % % N=[0.10 0.23 0.97];%外法向
 ...
 L=[0.32 0.93 0.19];%最大变化方向
N=[-0.94 0.29 0.17];%外法向
M=cross(N,L);
...
%% KHI
% % % tint2 = '2017-07-06T17:31:56.000Z/2017-07-06T17:31:58.000Z';
% % % dspan = 1;
% % % c_eval('gsmVe? = smoothdata(gsmVe?,''movmean'',dspan);',ic);
% % % c_eval('Ne? = smoothdata(Ne?,''movmean'',dspan);',ic);
% % % c_eval('B? = smoothdata(B?,''movmean'',dspan);',ic);
% % % c_eval('Vetmp = irf_tlim(gsmVe?,tint2);',ic);
% % % c_eval('Netmp = irf_tlim(Ne?,tint2);',ic);
% % % c_eval('Btmp = irf_tlim(B?,tint2);',ic);
% % % 
% % % c_eval('Vemean = mean(Vetmp(:,2:4))*1e3;',ic);
% % % c_eval('Nemean = mean(Netmp(:,2));',ic);
% % % c_eval('Bmean = mean(Btmp(:,2:4))*1e-9;',ic);
% % % % c_eval('k = (gsmVe1(:,2:4)*1e3-Vemean)./norm(gsmVe1(:,2:4)*1e3-Vemean);',ic);
% % % k = repmat([-0.8 0.6 0],size(gsmVe1,1),1);
% % % B1_rese = irf_resamp(B1,Vet1)*1e-9;
% % % rhomean = Nemean*1e6*units.me;
% % % c_eval('rho = Ne?(:,2)*1e6*units.me;',ic);
% % % rho_mean = rhomean*rho./(rho+rhomean);
% % % 
% % % % Vae = Bt1_rese(:,2)*1e-9./sqrt(units.mu0*units.me*Ne1(:,2)*1e6)/1e3; %km/s
% % % 
% % % left=irf_dot(k,(gsmVe1(:,2:4)*1e3-Vemean)).^2;
% % % right=(irf_dot(k,B1_rese(:,2:4)).^2+irf_dot(k,repmat(Bmean,size(k,1),1)).^2)./rho_mean/units.mu0;
%% Init figure
% ic = 1;
n=13;
i=1;
...
% set(gca,'Ylim',[-8 18], 'ytick',[-20:5:20]);
% irf_legend(gca,'c',[0.99 0.98],'color','k','fontsize',12);
...
h(i)=irf_subplot(n,1,-i);
% dspan = 5;
% c_eval('Vexb? = irf_resamp(Vexb?, gsmVe?);',ic);
% c_eval('gsmVe?(:,2:4) = gsmVe?(:,2:4) - Vexb?(:,2:4);',ic);
...
% irf_plot([Vet_smooth(:,1) Vet_smooth(:,2)], 'color','k', 'Linewidth',0.75); hold on;
...
%% Vi-Vexb plot
% % % h(i)=irf_subplot(n,1,-i);
% % % Vexb1_resVi = irf_resamp(Vexb1,Vi1);
% % % 
% % % irf_plot([Vi1(:,1) Vi1(:,2)-Vexb1_resVi(:,2)], 'color','b', 'Linewidth',0.75); hold on;
% % % irf_plot([Vi1(:,1) Vi1(:,3)-Vexb1_resVi(:,3)], 'color','g', 'Linewidth',0.75); hold on;
% % % irf_plot([Vi1(:,1) Vi1(:,4)-Vexb1_resVi(:,4)], 'color','r', 'Linewidth',0.75); hold on;
% % % 
% % % irf_plot([Vi1(:,1) Vi1(:,2)*0],'k--', 'Linewidth',0.75); hold off;
% % % grid off;
% % % ylabel('Vi-Vexb [km/s]','fontsize',10);
% % % % set(gca,'Ylim',[fix(min([min(Vexb1(:,2)) min(Vexb1(:,3)) min(Vexb1(:,4))]))-1 fix(max(max(Vexb1(:,2:4))))+1]);
% % % % set(gca,'Ylim',[-200 400], 'ytick',[-100 0 300]);
% % % % irf_legend(gca,'d',[0.99 0.98],'color','k','fontsize',12);
% % % % set(gca,'ColorOrder',[[0 0 1];[0 1 0];[1 0 0];[0 0 0];[1 0 1]]);
...
%% Ve-Vexb plot
% % % h(i)=irf_subplot(n,1,-i);
% % % Vexb1_resVe = irf_resamp(Vexb1,Ve1);
% % % 
% % % irf_plot([Ve1(:,1) Ve1(:,2)-Vexb1_resVe(:,2)], 'color','b', 'Linewidth',0.75); hold on;
% % % irf_plot([Ve1(:,1) Ve1(:,3)-Vexb1_resVe(:,3)], 'color','g', 'Linewidth',0.75); hold on;
% % % irf_plot([Ve1(:,1) Ve1(:,4)-Vexb1_resVe(:,4)], 'color','r', 'Linewidth',0.75); hold on;
% % % 
% % % irf_plot([Ve1(:,1) Ve1(:,2)*0],'k--', 'Linewidth',0.75); hold off;
% % % grid off;
% % % ylabel('Ve-Vexb [km/s]','fontsize',10);
% % % % set(gca,'Ylim',[fix(min([min(Vexb1(:,2)) min(Vexb1(:,3)) min(Vexb1(:,4))]))-1 fix(max(max(Vexb1(:,2:4))))+1]);
% % % % set(gca,'Ylim',[-200 400], 'ytick',[-100 0 300]);
% % % % irf_legend(gca,'d',[0.99 0.98],'color','k','fontsize',12);
% % % % set(gca,'ColorOrder',[[0 0 1];[0 1 0];[1 0 0];[0 0 0];[1 0 1]]);
...
% % % h(i)=irf_subplot(n,1,-i);
% % % dspan = 1;
% % % c_eval('Evixb?(:,1) = smooth(Evixb?(:,1), dspan);',ic);
% % % c_eval('Evixb?(:,2) = smooth(Evixb?(:,2), dspan);',ic);
% % % c_eval('Evixb?(:,3) = smooth(Evixb?(:,3), dspan);',ic);
% % % % c_eval('Evixb? = irf_abs(Evixb?);',ic);
...
% % % c_eval("irf_plot([E?_err(2:2:end,1) E?_err(2:2:end,2)], 'm--', 'Linewidth',0.75);",ic);hold on;
% % % c_eval("irf_plot([E?(:,1) E?(:,2)*0],'k--', 'Linewidth',0.75);",ic); hold off;
% % % grid off;
% % % set(gca,'Ylim',[-10 10]);
...
% % % dspan = 1;
% % % 
% % % c_eval('Evexb?(:,1) = smooth(Evexb?(:,1), dspan);',ic);
% % % c_eval('Evexb?(:,2) = smooth(Evexb?(:,2), dspan);',ic);
% % % c_eval('Evexb?(:,3) = smooth(Evexb?(:,3), dspan);',ic);
% % % c_eval('Evexb? = irf_abs(Evexb?);',ic);
...
% % % % c_eval("irf_plot([B?(:,1) Evexb?(:,4)], 'color','b', 'Linewidth',0.75);",ic);hold on;
% % % c_eval("irf_plot([E?_err(1:2:end,1) E?_err(1:2:end,2)], 'm--', 'Linewidth',0.75);",ic);hold on;
% % % c_eval("irf_plot([E?_err(2:2:end,1) E?_err(2:2:end,2)], 'm--', 'Linewidth',0.75);",ic);hold on;
% % % c_eval("irf_plot([E?(:,1) E?(:,2)*0],'k--', 'Linewidth',0.75);",ic); hold off;
% % % grid off;
...
%% Vit plot
% % % h(i)=irf_subplot(n,1,-i);
% % % c_eval("irf_plot([Vit?(:,1) Vit?(:,2)], 'color','k', 'Linewidth',0.5);",ic); hold on;
...
% % % grid off;
% % % c_eval("set(gca,'Ylim',[0 fix(max(Vit?(:,2))/10)*10+10],'fontsize',9);",ic);
% % % %set(gca,'Ylim',[-200 400], 'ytick',[-100 0 300]);
% % % % irf_legend(gca,'d',[0.99 0.98],'color','k','fontsize',12);
% % % % set(gca,'ColorOrder',[[0 0 1];[0 1 0];[1 0 0];[0 0 0];[1 0 1]]);
% % % % irf_legend(gca,{'Vi_N','Vi_M','Vi_L','|Vi|','|Vexb|'},[0.1 0.12]);
...
% c_eval('gsmVi?(:,2:4) = gsmVi?(:,2:4) - [360 250 -140];', ic);
...
set(gca,'Ylim',[-150 150], 'ytick',[-100 0 100]);
% irf_legend(gca,'d',[0.99 0.98],'color','k','fontsize',12);
% set(gca,'ColorOrder',[[0 0 1];[0 1 0];[1 0 0];[0 0 0];[1 0 1]]);
% irf_legend(gca,{'Vi_N','Vi_M','Vi_L','|Vi|','|Vexb|'},[0.1 0.12]);
set(gca,'ColorOrder',[[0 0 1];[0 1 0];[1 0 0];[0 0 0]]);
...
% % % c_eval("irf_plot([gradB(:,1) gradB(:,10)], 'color','r', 'Linewidth',0.75);",ic); hold on;
% % % 
% % % c_eval("irf_plot([gsmVi?(:,1) gsmVi?(:,2)*0],'k--', 'Linewidth',0.75);",ic); hold off;
% % % grid off;
% % % % c_eval("set(gca,'Ylim',[fix(min([min(gsmVi?(:,2)) min(gsmVi?(:,3)) min(gsmVi?(:,4))])/10)*10-10 fix(max(Vit?(:,2))/10)*10+10],'fontsize',9);",ic);
...
% % % set(gca,'ColorOrder',[[0 0 1];[0 1 0];[1 0 0];[0 0 0]]);
% % % % irf_legend(gca,{'Vi_x','Vi_y','Vi_z'},[0.97 0.92]);
% % % ylabel('dBz/dx [nT/km]','fontsize',8);
% % % i=i+1;
%% dBz/dz
% % % h(i)=irf_subplot(n,1,-i);
% % % c_eval("irf_plot([gradB(:,1) gradB(:,10)], 'color','k', 'Linewidth',0.75);",ic); hold on;
% % % 
% % % c_eval("irf_plot([gsmVi?(:,1) gsmVi?(:,2)*0],'k--', 'Linewidth',0.75);",ic); hold off;
% % % grid off;
% % % % c_eval("set(gca,'Ylim',[fix(min([min(gsmVi?(:,2)) min(gsmVi?(:,3)) min(gsmVi?(:,4))])/10)*10-10 fix(max(Vit?(:,2))/10)*10+10],'fontsize',9);",ic);
...
% % % set(gca,'ColorOrder',[[0 0 1];[0 1 0];[1 0 0];[0 0 0]]);
% % % % irf_legend(gca,{'Vi_x','Vi_y','Vi_z'},[0.97 0.92]);
% % % ylabel('dBz/dz [nT/km]','fontsize',8);
% % % i=i+1;
%% Vifac plot
% % % h(i)=irf_subplot(n,1,-i);
...
% % % grid off;
% % % c_eval("set(gca,'Ylim',[min([min(J_B(:,2:4))])-1 max([max(J_B(:,5))])+1],'fontsize',9);",ic);
% % % set(gca,'Ylim',[-120 150]);
% % % % irf_legend(gca,'d',[0.99 0.98],'color','k','fontsize',12);
% % % % set(gca,'ColorOrder',[[0 0 1];[0 1 0];[1 0 0];[0 0 0];[1 0 1]]);
% % % % irf_legend(gca,{'Vi_N','Vi_M','Vi_L','|Vi|','|Vexb|'},[0.1 0.12]);
...
%% JdotE
% % % h(i)=irf_subplot(n,1,-i);
% % % c_eval('JdotE? = irf_resamp(JdotE?,Ve?);',ic);
% % % % dspan = 20;
...
% % % set(gca,'Ylim',[-50 50], 'ytick',[-50 0 50]);
% % % % irf_legend(gca,'d',[0.99 0.98],'color','k','fontsize',12);
% % % % set(gca,'ColorOrder',[[0 0 1];[0 1 0];[1 0 0];[0 0 0];[1 0 1]]);
% % % % irf_legend(gca,{'Vi_N','Vi_M','Vi_L','|Vi|','|Vexb|'},[0.1 0.12]);
% % % set(gca,'ColorOrder',[0 0 0]);
...
%% J dot E' plot
h(i)=irf_subplot(n,1,-i);
% c_eval('JdotEplus?(:,1) = smooth(JdotEplus?(:,1),dspan);',ic);c_eval('JdotEplus?(:,2) = smooth(JdotEplus?(:,2),dspan);',ic);
...
%% KHI(Vae)
% h(i)=irf_subplot(n,1,-i);
% c_eval("irf_plot([Ne?(:,1) left], 'color','b', 'Linewidth',0.75);",ic); hold on;
% c_eval("irf_plot([Vet?(:,1) right], 'color','r', 'Linewidth',0.75);",ic); hold on;
% % c_eval("irf_plot([JdotE?(:,1) JdotE?(:,2)*0],'k--', 'Linewidth',0.75);",ic); hold off;
% grid off;
% % c_eval("set(gca,'Ylim',[fix(min(JdotE?)/10)*10-10 fix(max(JdotE?)/10)*10+10],'fontsize',9);",ic);
% % set(gca,'Ylim',[-500 300], 'ytick',[-300 0 300]);
% % irf_legend(gca,'d',[0.99 0.98],'color','k','fontsize',12);
% % set(gca,'ColorOrder',[[0 0 1];[0 1 0];[1 0 0];[0 0 0];[1 0 1]]);
% % irf_legend(gca,{'Vi_N','Vi_M','Vi_L','|Vi|','|Vexb|'},[0.1 0.12]);
% set(gca,'ColorOrder',[0 0 0]);
...
% % % % c_eval("irf_plot([Vit?(:,1) Pb?], 'color','g', 'Linewidth',0.75);",ic); hold on;
...
% % % i=i+1; 
%% Interchange instability criterion
% % % 
% % % h(i)=irf_subplot(n,1,-i);
...
specrec_p_elow.p_label={' ','log10(keV/(cm^2 s sr keV))'};
...
specrec_p_emid.p_label={' ','log10(keV/(cm^2 s sr keV))'};
...
specrec_p_ehigh.p_label={' ','log10(keV/(cm^2 s sr keV))'};
...
% colormap(h(i),jet)
% 
% specrec_p_e=struct('t',irf_time(Flux_e_feeps.time.epoch,'ttns>epoch'));
% specrec_p_e.f=transpose(Flux_e_feeps.depend{1, 1}(1,1:end));%energy levels
% specrec_p_e.p=Flux_e_feeps.data;%data matrix
% specrec_p_e.f_label='';
% specrec_p_e.p_label={' ','1/(cm^2 s sr keV)'};
% [h(i), hcb_feeps_e]=irf_spectrogram(h(i),specrec_p_e);
% % hold on;
% % irf_plot([Energy_exb1(:,1) Energy_exb1(:,2)], 'color','k', 'Linewidth',0.75); hold off;
% grid off;
% set(h(i),'yscale','log');
% set(h(i),'ytick',[4e4 1e5 5e5],'fontsize',9);
% ylabel('Ee(ev)','fontsize',8)
% set(gca,'Ylim',[4.8e4 5.8e5]);
% caxis(gca,[-1.5 4])
% set(gca,'xtick',[])
% % irf_legend(gca,'f',[0.99 0.98],'color','k','fontsize',12);
% poscbar_feeps_e=get(hcb_feeps_e,'pos');
% poscbar_feeps_e(3)=poscbar_feeps_e(3)*0.5;
% %poscbar6(1)=poscbar6(1)*0.5;
% set(hcb_feeps_e,'pos',poscbar_feeps_e);
% i=i+1;
