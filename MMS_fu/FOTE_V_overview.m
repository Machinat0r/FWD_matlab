...
[j,divB,B,jxB,divTshear,divPb] = c_4_j('R?','B?');
% d_cros_ve=c_4_grad('R?','Ve?(:,1:4)','curl');
...
h(6)=irf_panel('VeLMN');
hold(h(6),'on');
irf_plot(h(6),[VeSS1(:,1),VeSS1(:,2)],'Linewidth',lnwid);
irf_plot(h(6),[VeSS1(:,1),VeSS1(:,3)],'Linewidth',lnwid);
irf_plot(h(6),[VeSS1(:,1),VeSS1(:,4)],'Linewidth',lnwid);
...
irf_plot(h(8),[Ve2(:,1),Ve2(:,3)],'Linewidth',lnwid,'color','r');
hold(h(8),'off');
...
set(h(10),'Ylim',[0 100],'Ytick',[0 40 100]);
