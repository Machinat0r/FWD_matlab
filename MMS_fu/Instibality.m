...
SDCFilesDownload_NAS(filenames_srvy,TempDir, 'Threads', 48, 'CheckSize', 0)
...
% % % % c_eval(['gsmVe?_ts=irf_gse2gsm1(Ve?_ts);'],ic);
...
B0 = [15.7,8.6,69.5];
for iii = 1:size(B1,1)
    phi(iii) = acos(dot(B0,B1(iii,2:4))/norm(B0)/norm(B1(iii,2:4)));
end

% Bomni = nanmean(B_omni(:,2:4),1);
% theta = acosd(dot(B0,Bomni)/norm(B0)/norm(Bomni));

% Cs = sqrt(gamma*p/rho)
Cs = [Te1(:,1) sqrt((Te1(:,2)*e+3.*Ti1(:,2)*e)./Mp)];  %m/s
Ni1_Bres = irf_resamp(Ni1,Bt1);
Va = [B1(:,1) 22*Bt1(:,2)./sqrt(Ni1_Bres(:,2))*1e3];

Wb2 = Cs;
Wb2(:,2) = g^2./Cs(:,2).^2+g/L;

phi = pi/30;
alpha = phi/L;
LHS = [Va(:,1) Va(:,2).^2.*alpha.^2/4];
LHS = irf_resamp(LHS, Wb2);
Cri = [Wb2(:,1) Wb2(:,2)-LHS(:,2)];
%% Init figure
n=5;
i=1;
...
c_eval("irf_plot([Cri(:,1) Cri(:,2)], 'color','b', 'Linewidth',0.75);",ic);hold on;
grid off;
c_eval("irf_plot([Bt?(:,1) 0*Bt?(:,2)],'k--', 'Linewidth',0.75);",ic); hold off;
set(h(3),'Ylim',[-1e-3, 1e-3]);
%  set(gca,'ColorOrder',[[0 0 1];[0 1 0]]);
%  irf_legend(gca,{'Ne','Ni'},[0.1 0.12]);
  % set(gca,'ColorOrder',[[0 0 1];[0 1 0]]);
% irf_legend(gca,{'B_N'},[pos2(1)+1.15*pos2(3),pos2(2)]);
% irf_legend(gca,'b',[0.99 0.98],'color','k','fontsize',12)
ylabel('Criterion','fontsize',8);
i=i+1;
%% Ve plot
% % h(i)=irf_subplot(n,1,-i);
% % dspan = 4;
% % % c_eval('gsmVe? = [smooth(Ve?(:,1), dspan), smooth(gsmVe?(:,2), dspan), smooth(gsmVe?(:,3), dspan), smooth(gsmVe?(:,4), dspan)];',ic);
