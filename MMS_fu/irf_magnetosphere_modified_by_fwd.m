function [x,y,omni] = irf_magnetosphere_modified_by_fwd(model,Dp,Bz)
...
switch lower(model)
    case 'mp_petrinec1996'
    % Reference: Petrinec and Russell 1996
    theta=0:0.1:pi;
    if Bz>0, m1=0; else, m1=0.16;end
    r=14.63*(Dp./2.1)^(-1/6)./(1+cos(theta).*(14.63/(10.3+m1.*Bz)-1));
    ...
    case 'mp_sibeck1991'
    % Reference: Sibeck et al 1991
    x=-50:0.1:20;
    A=0.14;B=18.2;C=-217.2;p0=2.04;
    yy = -A*x.^2-B*(p0/Dp)^1/6.*x-C*(p0/Dp)^1/3;
    x(yy<0)=[];
    yy(yy<0)=[];
    y=sqrt(yy);
