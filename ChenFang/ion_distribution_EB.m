npanels = 6;
tint = irf.tint('2017-07-17T07:53:05.00Z/2017-07-17T07:53:06.50Z');
% tint = irf.tint('2017-06-19T04:02:53.00Z/2017-06-19T04:02:58.00Z');
tintZoom = tint;

cmap = 'jet';
[h1,h2] = initialize_combined_plot(npanels,3,1,0.4,'vertical');
ic = 1:4;
iisub = 0;
cmap = colormap('jet');
...
tintDist = irf.tint('2017-07-17T07:53:05.00Z/2017-07-17T07:53:06.50Z');
c_eval('dist? = iPDist?.convertto(''s^3/km^6'');',ic)
c_eval('dist_scm? = iPDist?;',ic)
c_eval('scpot? = scPot?;',ic)
c_eval('dmpaB?slow = dmpaB?.resample(gseVi?);',ic)

c_eval('dslE?slow = dslE?.resample(gseVi?);',ic)
%c_eval('ePitch = ePitch?.convertto(''s^3/km^6'');',ic)

% Plot format input
vlim = 2*1e3;%jingdu(diandaomian)
elevlim = 3;
strCMap = 'jet';
% projclim = [-1 2];  
 projclim_int = [-10 -7.5];%sebiaofanwei
% projclim_int = [-1 2]; 

c_eval('times = iPDist1.time;',ic)
