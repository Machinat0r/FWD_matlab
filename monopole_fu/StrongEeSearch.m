...
Date = '2022-07-11T12:26:33.1Z/2022-07-11T13:30:34.9Z';
...
try
    ...
    Pos = mms.get_data('R_gsm',tempTint,1);
    RE = units.RE;
    R_RE = mean(Pos.data(:,4)/RE*1000,1);
%R判据
    if R_RE < 8, continue; end
    if irf_time(Pos.time.epoch(end),'ttns>epochTT').epoch <= tempTint.epoch(2)
        tint = tempTint;
    else
        tint = irf.tint(irf_time(Pos.time.epoch(1),'ttns>epochTT'),irf_time(Pos.time.epoch(end),'ttns>epochTT'));
    end
