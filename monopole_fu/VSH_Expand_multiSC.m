function [coeff, res] = VSH_Expand_multiSC(varargin)
...
%% Resample
c_eval('B? = irf_resamp(B?, B1);', 2:5);
c_eval('R? = irf_resamp(R?, B1);', 1:5);
c_eval('B?_sph = B?;', 1:5);
c_eval('R?_sph = R?;', 1:5);
...
c_eval('R?(:,2:4) = R?(:,2:4) - SphereCenter;', 1:5);
...
[B5_sph, R5_sph] = Coor_Trans(B5(:,2:4), R5(:,2:4)); %#ok
c_eval('B?_sph = [B?(:,1), B?_sph];', 1:5);
c_eval('R?_sph = [R?(:,1), R?_sph];', 1:5);
end
...
[coeff(temp_t,:), res(temp_t,:)] = SPH_coef(B1_sph(temp_t,2:4), B2_sph(temp_t,2:4), B3_sph(temp_t,2:4),...
    B4_sph(temp_t,2:4), B5_sph(temp_t,2:4), R1_sph(temp_t,2:4), R2_sph(temp_t,2:4), R3_sph(temp_t,2:4),...
    R4_sph(temp_t,2:4), R5_sph(temp_t,2:4), flag); %#ok
end
...
function [coeff, res] = SPH_coef(B1, B2, B3, B4, B5, R1, R2, R3, R4, R5, flag)
if flag == 1
% one-step
c_eval('r? = R?(:,1);',1:4);
...
c_eval('R? = R? - R0;', 1:5);
[B1, R1_temp] = Coor_Trans(B1, R1); [B2, R2_temp] = Coor_Trans(B2, R2);
[B3, R3_temp] = Coor_Trans(B3, R3); [B4, R4_temp] = Coor_Trans(B4, R4); [B5, R5_temp] = Coor_Trans(B5, R5);
maxR = 0;
c_eval('maxR = max(maxR,R?_temp(:,1));',1:5);
c_eval('R?_temp_norm = R?_temp(:,1)./maxR;',1:5);
alpha_00 = coeff0(1); beta_00 = coeff0(2); gamma_00 = coeff0(3);
for ic = 1:5
temp_ic = num2str(ic);
eval(['theta',temp_ic,' = R',temp_ic,'_temp(2) + eps;']);
...
B1_res = [Br01, Bt01, Bp01] - B1; B2_res = [Br02, Bt02, Bp02] - B2;
B3_res = [Br03, Bt03, Bp03] - B3; B4_res = [Br04, Bt04, Bp04] - B4; B5_res = [Br05, Bt05, Bp05] - B5;
c_eval('B? = B?_res;',1:5);
c_eval('B?x = B?(1)*sin(R?_temp(2))*cos(R?_temp(3)) + B?(2)*cos(R?_temp(2))*cos(R?_temp(3)) - B?(3)*sin(R?_temp(3));', 1:5);
c_eval('B?y = B?(1)*sin(R?_temp(2))*sin(R?_temp(3)) + B?(2)*cos(R?_temp(2))*sin(R?_temp(3)) + B?(3)*cos(R?_temp(3));', 1:5);
c_eval('B?z = B?(1)*cos(R?_temp(2)) - B?(2)*sin(R?_temp(2));', 1:5);
c_eval('B? = -[B?x, B?y, B?z];', 1:5);
...
c_eval('maxR = max(maxR,R?_temp(:,1));',1:5);
c_eval('R?_temp_norm = R?_temp(:,1)./maxR;',1:5);
alpha_12 = coeff1(1); alpha_10 = coeff1(2); alpha_11 = coeff1(3); %#ok
beta_12 = coeff1(4); beta_10 = coeff1(5); beta_11 = coeff1(6); %#ok
gamma_12 = coeff1(7); gamma_10 = coeff1(8); gamma_11 = coeff1(9); %#ok
for ic = 1:5
temp_ic = num2str(ic);
eval(['theta',temp_ic,' = R',temp_ic,'_temp(2) + eps;']);
eval(['phi',temp_ic,' = R',temp_ic,'(3) + eps;']);
eval(['Br1',temp_ic,' = Cal_Br1(R',temp_ic,'_temp_norm,theta',temp_ic,',phi',temp_ic,', alpha_12, alpha_10, alpha_11);']);
eval(['Bt1',temp_ic,' = Cal_Bt1(R',temp_ic,'_temp_norm,theta',temp_ic,',phi',temp_ic,', beta_12, beta_10, beta_11, gamma_12, gamma_11);']);
eval(['Bp1',temp_ic,' = Cal_Bp1(R',temp_ic,'_temp_norm,theta',temp_ic,',phi',temp_ic,', beta_12, beta_11, gamma_12, gamma_10, gamma_11);']);
end
B1_res = [Br11, Bt11, Bp11] - B1; B2_res = [Br12, Bt12, Bp12] - B2;
B3_res = [Br13, Bt13, Bp13] - B3; B4_res = [Br14, Bt14, Bp14] - B4; B5_res = [Br15, Bt15, Bp15] - B5;
c_eval('B? = B?_res;',1:5);
c_eval('B?x = B?(1)*sin(R?_temp(2))*cos(R?_temp(3)) + B?(2)*cos(R?_temp(2))*cos(R?_temp(3)) - B?(3)*sin(R?_temp(3));', 1:5);
c_eval('B?y = B?(1)*sin(R?_temp(2))*sin(R?_temp(3)) + B?(2)*cos(R?_temp(2))*sin(R?_temp(3)) + B?(3)*cos(R?_temp(3));', 1:5);
c_eval('B?z = B?(1)*cos(R?_temp(2)) - B?(2)*sin(R?_temp(2));', 1:5);
...
options = optimoptions('lsqnonlin','Algorithm','levenberg-marquardt','MaxFunctionEvaluations',1e4,...
    'MaxIterations',1e4);
x2 = zeros(30,1);
[coeff2,~,res,~] = lsqnonlin(@(coeff) Cal_func2(B1, B2, B3, B4, B5, R1, R2, R3, R4, R5, coeff), x2,[],[],options);
coeff = [coeff0(1); coeff1(1:3); coeff2(1:5);...
         coeff0(2); coeff1(4:6); coeff2(6:10);...
         coeff0(3); coeff1(7:9); coeff2(11:15);...
         coeff0(4:6)];
...
function func = Cal_func2(B1, B2, B3, B4, B5, R1, R2, R3, R4, R5, coeff) %#ok
% alpha_12 = alpha1-1
alpha_24 = coeff(1); alpha_23 = coeff(2); alpha_20 = coeff(3); alpha_21 = coeff(4); alpha_22 = coeff(5);%#ok
beta_24 = coeff(6); beta_23 = coeff(7); beta_20 = coeff(8); beta_21 = coeff(9); beta_22 = coeff(10);%#ok
gamma_24 = coeff(11); gamma_23 = coeff(12); gamma_20 = coeff(13); gamma_21 = coeff(14); gamma_22 = coeff(15);%#ok
% Rx0 = coeff(10); Ry0 = coeff(11); Rz0 = coeff(12);
% R0 = [Rx0, Ry0, Rz0]; %#ok
R0 = [0, 0, 0];
...
maxR = 0;
c_eval('maxR = max(maxR,r?(:,1));',1:5);
c_eval('r? = r?(:,1)./maxR;',1:5);

for ic = 1:5
temp_ic = num2str(ic);
eval(['Br2',temp_ic,' = Cal_Br2(r',temp_ic,', theta',temp_ic,', phi',temp_ic,', alpha_24, alpha_23, alpha_20, alpha_21, alpha_22);']);
eval(['Bt2',temp_ic,' = Cal_Bt2(r',temp_ic,', theta',temp_ic,', phi',temp_ic,', gamma_24, gamma_23, gamma_21, gamma_22, beta_24, beta_23, beta_20, beta_21, beta_22);']);
eval(['Bp2',temp_ic,' = Cal_Bp2(r',temp_ic,', theta',temp_ic,', phi',temp_ic,', gamma_24, gamma_23, gamma_20, gamma_21, gamma_22, beta_24, beta_23, beta_21, beta_22);']);
end

func = [Br21 - B1(1); Br22 - B2(1); Br23 - B3(1); Br24 - B4(1); Br25 - B5(1);
        Bt21 - B1(2); Bt22 - B2(2); Bt23 - B3(2); Bt24 - B4(2); Bt25 - B5(2);
        Bp21 - B1(3); Bp22 - B2(3); Bp23 - B3(3); Bp24 - B4(3); Bp25 - B5(3);];
end
%% component function
...
function Br2 = Cal_Br2(r, theta, phi, alpha_24, alpha_23, alpha_20, alpha_21, alpha_22)
Br2 = r.^-4 .* (alpha_24 .* cos(-2 * phi) .* Cal_SPH(2, -2, theta)...
    + alpha_23 .* cos(-1 * phi) .* Cal_SPH(2, -1, theta) ...
    + alpha_20 .* cos(0 * phi) .* Cal_SPH(2, 0, theta) ... 
    + alpha_21 .* cos(1 * phi) .* Cal_SPH(2, 1, theta) ...
    + alpha_22 .* cos(2 * phi) .* Cal_SPH(2, 2, theta));
end

function Bt2 = Cal_Bt2(r, theta, phi, gamma_24, gamma_23, gamma_21, gamma_22, beta_24, beta_23, beta_20, beta_21, beta_22)
Bt2 = r.^-3 .* (gamma_24 .* 2 .* cos(2 * phi) ./ (3*sin(theta)) .* Cal_SPH(2, -2, theta)...
    + beta_24 .* sin(2 * phi)./ 3 .* Cal_dPlm(2, -2, theta)...
    + gamma_23 .* cos(phi) ./ (3*sin(theta)) .* Cal_SPH(2, -1, theta)...
    + beta_23 .* sin(phi)./ 3 .* Cal_dPlm(2, -1, theta)...
    + beta_20 ./ 3 .* Cal_dPlm(2, 0, theta)...
    + gamma_21 .* -sin(phi) ./ (3*sin(theta)) .* Cal_SPH(2, 1, theta)...
    + beta_21 .* cos(phi)./ 3 .* Cal_dPlm(2, 1, theta)...
    + gamma_22 .* -2* sin(2 * phi) ./ (3*sin(theta)) .* Cal_SPH(2, 2, theta)...
    + beta_22 .* cos(2 * phi)./ 3 .* Cal_dPlm(2, 2, theta));
end

function Bp2 = Cal_Bp2(r, theta, phi, gamma_24, gamma_23, gamma_20, gamma_21, gamma_22, beta_24, beta_23, beta_21, beta_22)
Bp2 = r.^-3 .* (...
      gamma_24 .* sin(2 * phi) ./ 3 .* Cal_dPlm(2, -2, theta)...
    - beta_24 .* 2 .* cos(2 * phi)./ (3*sin(theta)) .* Cal_SPH(2, -2, theta)...
    + gamma_23 .* sin(phi) ./ 3 .* Cal_dPlm(2, -1, theta)...
    - beta_23 .* cos(phi)./ (3 * sin(theta)) .* Cal_SPH(2, -1, theta)...
    + gamma_20 ./ 3 .* Cal_dPlm(2, 0, theta)...
    + gamma_21 .* cos(phi) ./ 3 .* Cal_dPlm(2, 1, theta)...
    - beta_21 .* -sin(phi)./ (3 * sin(theta)) .* Cal_SPH(2, 1, theta)...
    + gamma_22 .* cos(2 * phi) ./ 3 .* Cal_dPlm(2, 2, theta)...
    - beta_22 .* -2 .* sin(2 * phi)./ (3*sin(theta)) .* Cal_SPH(2, 2, theta));
end
%% R normalization
