%% ACE 2024 May superstorm, IRF-MATLAB 7-panel script
...
eventTag = '20240510T0000_20240512T0000';

mfiCsv1 = fullfile(cacheDir, 'AC_H3_MFI_20240510_Magnitude_BGSEc.csv');
mfiFile1 = fullfile(cacheDir, 'AC_H3_MFI_20240510_Magnitude_BGSEc.mat');
mfiCsv2 = fullfile(cacheDir, 'AC_H3_MFI_20240511_Magnitude_BGSEc.csv');
mfiFile2 = fullfile(cacheDir, 'AC_H3_MFI_20240511_Magnitude_BGSEc.mat');
...
epmUrl = ['https://cdaweb.gsfc.nasa.gov/hapi/data?id=AC_H3_EPM', ...
    '&parameters=' epmParams '&time.min=2024-05-10T00:00:00Z', ...
    '&time.max=2024-05-12T00:00:00Z&format=csv'];

cacheJobs = {mfiCsv1,mfiFile1,mfiUrl1; mfiCsv2,mfiFile2,mfiUrl2; ...
    sweCsv,sweFile,sweUrl; ...
...
mfiPart1 = load(mfiFile1);
mfiPart2 = load(mfiFile2);
mfi.t = [mfiPart1.t(:); mfiPart2.t(:)];
mfi.y = [mfiPart1.y; mfiPart2.y];  % [|B| Bx By Bz], 1-s H3
...
% Use LEMS120 because LEMS30 P1-P6 have been unreliable since late 2003.
J_i = yEpm(:, 17:24);
J_e = yEpm(:, 9:12);
...
fig = gcf;
set(fig, 'Color', 'w', 'Name', 'ACE 2024 May superstorm IRF 7-panel', ...
...
xlabel(h(end), '2024-05-10 to 12 UTC');
...
outFig = fullfile(baseDir, 'ace_20240510_12_solarwind_overview.fig');
outPng = fullfile(baseDir, 'ace_20240510_12_solarwind_overview.png');
outPdf = fullfile(baseDir, 'ace_20240510_12_solarwind_overview.pdf');
