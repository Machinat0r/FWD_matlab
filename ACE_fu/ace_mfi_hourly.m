...
%   1. Select exactly one native product/file for each UTC hour. A UTC day
%      may therefore use different products in different hours.
%   2. First pass: choose the first product in H3 > H0 > H1 > H2 priority
%      whose valid-sample coverage reaches MinimumCoverageFraction.
%   3. Second pass: if no product reaches that threshold, choose the candidate
%      with the greatest non-zero coverage; product priority breaks ties.
%   4. Never resample, interpolate, aggregate one product to another cadence,
%      or combine samples from two products within the same hourly statistic.
%   5. Calculate mean(Magnitude) and max(Magnitude) directly from the selected
%      file's native samples in each half-open UTC hour [hh:00, hh+1:00).
...
%   'MinimumCoverageFraction' 0.5 (default); primary-pass threshold
...
%       "C:\ACE_MFI_hourly_output");
    ...
    if isempty(Manifest)
        error('ACE:NoMfiFiles', 'No ACE MFI H0-H3 CDF files were found under %s.', mfiRoot);
    end
    ...
    if opts.Verbose
        ...
        fprintf(['Native hourly selection: H3 > H0 > H1 > H2; ' ...
            'minimum primary coverage %.3f; no resampling.\n'], ...
            opts.MinimumCoverageFraction);
    end
    ...
    RunInfo.H3SelectedHours = sum(double(DailySelection.H3_hours));
    RunInfo.H0SelectedHours = sum(double(DailySelection.H0_hours));
    RunInfo.H1SelectedHours = sum(double(DailySelection.H1_hours));
    RunInfo.H2SelectedHours = sum(double(DailySelection.H2_hours));
    RunInfo.MissingHours = sum(double(DailySelection.Missing_hours));
    RunInfo.SufficientSelectedHours = sum(double(DailySelection.Sufficient_hours));
    RunInfo.BestAvailableSelectedHours = sum(double(DailySelection.Best_available_hours));
    RunInfo.DaysUsingH3 = nnz(DailySelection.H3_hours > 0);
    RunInfo.DaysUsingH0 = nnz(DailySelection.H0_hours > 0);
    RunInfo.DaysUsingH1 = nnz(DailySelection.H1_hours > 0);
    RunInfo.DaysUsingH2 = nnz(DailySelection.H2_hours > 0);
    RunInfo.FullyMissingDays = nnz(DailySelection.Missing_hours == 24);
    RunInfo.PartialDays = nnz(DailySelection.Missing_hours > 0 & ...
        DailySelection.Missing_hours < 24);
    ...
    RunInfo.SelectionRule = sprintf([ ...
        'One native file per UTC hour: first H3>H0>H1>H2 candidate with ' ...
        'coverage >= %.3f; otherwise greatest non-zero coverage, priority on ties'], ...
        opts.MinimumCoverageFraction);
    ...
    if opts.Verbose
        fprintf('\nCompleted.\n');
        fprintf(['Selected hours: H3=%d; H0=%d; H1=%d; H2=%d; ' ...
            'missing=%d\n'], RunInfo.H3SelectedHours, RunInfo.H0SelectedHours, ...
            RunInfo.H1SelectedHours, RunInfo.H2SelectedHours, RunInfo.MissingHours);
...
function opts = parseOptions(varargin)
    ...
    addParameter(p, 'MinimumCoverageFraction', 0.5, ...
    ...
    opts.MinimumCoverageFraction = double(opts.MinimumCoverageFraction);
...
function mfiRoot = locateMfiRoot(dataRoot)
    ...
    candidate2 = dataRoot;
    if hasAnyMfiProduct(candidate1)
        mfiRoot = candidate1;
    elseif hasAnyMfiProduct(candidate2)
        mfiRoot = candidate2;
    else
        error('ACE:MfiRootNotFound', ...
            ['Could not find any ace\mfi\h0-h3\l2 product under %s. ' ...
...
function tf = hasAnyMfiProduct(folder)
    tf = false;
    for product = ["h0", "h1", "h2", "h3"]
        if isfolder(fullfile(folder, product, "l2"))
            tf = true;
            return;
        end
    end
...
function Manifest = buildManifest(mfiRoot)
    patterns = [ ...
        ...
        fullfile(mfiRoot, "h1", "l2", "**", "ac_h1_mfi_*.cdf"), ...
        fullfile(mfiRoot, "h2", "l2", "**", "ac_h2_mfi_*.cdf")];
    ...
    for i = 1:numel(listings)
        ...
        for j = 1:numel(items)
            token = regexp(items(j).name, ...
                '^ac_(h[0-3])_mfi_(\d{8})_v(\d+)\.cdf$', 'tokens', 'once');
            if isempty(token)
                continue;
            end
            row = row + 1;
...
function config = processingConfig(opts)
    config.CacheFormatVersion = uint32(3);
    config.Signature = string(sprintf([ ...
        'ACE_MFI_NATIVE_HOURLY_V3_Q0_MAG_0_500_HOUR_LEVEL_' ...
        'H3_H0_H1_H2_MINCOV_%.6f'], opts.MinimumCoverageFraction));
    config.ValidMagnitudeMin = 0;
    config.ValidMagnitudeMax = 500;
    config.ValidQualityFlag = 0;
    config.MinimumCoverageFraction = opts.MinimumCoverageFraction;
...
function [YearHourly, YearDaily, YearLog] = processYear( ...
    ...
    SourceFile = strings(nHours, 1);
    SelectionModeCode = zeros(nHours, 1, 'uint8');
        ...
        try
            ...
            if cacheMetaMatches(loaded.CacheMeta, expectedMeta)
                ...
                if checkpointIsValid(C, nHours, nDays) && ...
                    ...
                    newSourceFile = C.SourceFile;
                    newSelectionModeCode = C.SelectionModeCode;
                    ...
                    SelectionModeCode = newSelectionModeCode;
                    YearLog = newYearLog;
    ...
    productNames = ["H3", "H0", "H1", "H2"];
    productCodes = uint8([1, 2, 3, 4]);

    for dayIndex = nextDayIndex:nDays
        dateUTC = datesThisYear(dayIndex);
        hourRows = (dayIndex - 1) * 24 + (1:24);
        candidatePools = cell(numel(productNames), 1);
        hadAnyFile = false;
        for productIndex = 1:numel(productNames)
            product = productNames(productIndex);
            candidates = manifestThisYear( ...
                manifestThisYear.DateUTC == dateUTC & ...
                manifestThisYear.Product == product, :);
            hadAnyFile = hadAnyFile || ~isempty(candidates);
            [candidatePools{productIndex}, YearLog] = readCandidates( ...
                dateUTC, product, productCodes(productIndex), candidates, ...
                config, YearLog);
            % Once every hour has a sufficient candidate among products read
            % so far, no lower-priority product can be selected. Avoid opening
            % unnecessary lower-cadence CDFs and avoid irrelevant I/O failures.
            if all(hoursWithSufficientCandidate(candidatePools, ...
                    config.MinimumCoverageFraction))
                break;
            end
        end

        for hourIndex = 1:24
            [selected, selectionModeCode] = selectHourCandidate( ...
                candidatePools, hourIndex, config.MinimumCoverageFraction);
            if ~selected.Selected
                continue;
            end

            row = hourRows(hourIndex);
            result = selected.Result;
            BMean(row) = result.BMean(hourIndex);
            BMax(row) = result.BMax(hourIndex);
            NTotal(row) = result.NTotal(hourIndex);
            NValid(row) = result.NValid(hourIndex);
            Coverage(row) = result.Coverage(hourIndex);
            SourceCode(row) = selected.ProductCode;
            SourceVersion(row) = selected.Version;
            SourceFile(row) = selected.Path;
            SelectionModeCode(row) = selectionModeCode;
        end

        if all(SourceCode(hourRows) == 0)
            if hadAnyFile
                missingMessage = "All listed H0-H3 files were unreadable or had no valid samples.";
            else
                missingMessage = "No H0-H3 file is present for this UTC day.";
            end
            YearLog = appendLog(YearLog, dateUTC, "NONE", 0, "", ...
                "MISSING_DAY", missingMessage);
        end

        if opts.Verbose && (dayIndex == 1 || dayIndex == nDays || mod(dayIndex, 30) == 0)
            dayCodes = SourceCode(hourRows);
            fprintf(['[%04d] Processed day %d/%d (%s): ' ...
                'H3=%d H0=%d H1=%d H2=%d missing=%d\n'], ...
                thisYear, dayIndex, nDays, char(string(dateUTC, 'yyyy-MM-dd')), ...
                nnz(dayCodes == 1), nnz(dayCodes == 2), nnz(dayCodes == 3), ...
                nnz(dayCodes == 4), nnz(dayCodes == 0));
        end
        ...
        if dayIndex < nDays && mod(dayIndex, config.CheckpointEveryDays) == 0
            ...
            Checkpoint.SelectionModeCode = SelectionModeCode;
            Checkpoint.YearLog = YearLog;
            atomicSaveCheckpoint(checkpointFile, Checkpoint, expectedMeta);
        end
    end

    Source = categorical(double(SourceCode), [0, 1, 2, 3, 4], ...
        {'MISSING', 'H3', 'H0', 'H1', 'H2'});
    SelectionMode = categorical(double(SelectionModeCode), [0, 1, 2], ...
        {'MISSING', 'SUFFICIENT', 'BEST_AVAILABLE'});
    YearHourly = timetable(hourUTC, BMean, BMax, Source, SourceVersion, ...
        SourceFile, SelectionMode, NTotal, NValid, Coverage, ...
        'VariableNames', {'B_mean_nT', 'B_max_nT', 'Source', 'SourceVersion', ...
                          'SourceFile', 'SelectionMode', 'N_total', 'N_valid', ...
                          'Coverage'});
    YearHourly.Properties.DimensionNames{1} = 'TimeUTC';

    sourceByDay = reshape(SourceCode, 24, nDays);
    modeByDay = reshape(SelectionModeCode, 24, nDays);
    validByDay = reshape(NValid, 24, nDays);
    H3Hours = uint8(sum(sourceByDay == 1, 1).');
    H0Hours = uint8(sum(sourceByDay == 2, 1).');
    H1Hours = uint8(sum(sourceByDay == 3, 1).');
    H2Hours = uint8(sum(sourceByDay == 4, 1).');
    MissingHours = uint8(sum(sourceByDay == 0, 1).');
    SufficientHours = uint8(sum(modeByDay == 1, 1).');
    BestAvailableHours = uint8(sum(modeByDay == 2, 1).');
    DailyValidSamples = uint32(sum(double(validByDay), 1).');
    DailyStatus = strings(nDays, 1);
    for dayIndex = 1:nDays
        if MissingHours(dayIndex) == 24
            DailyStatus(dayIndex) = "MISSING";
        elseif MissingHours(dayIndex) > 0
            DailyStatus(dayIndex) = "PARTIAL";
        elseif BestAvailableHours(dayIndex) > 0
            DailyStatus(dayIndex) = "COMPLETE_WITH_BEST_AVAILABLE";
        else
            DailyStatus(dayIndex) = "COMPLETE_SUFFICIENT";
        end
    end
    YearDaily = table(datesThisYear, H3Hours, H0Hours, H1Hours, H2Hours, ...
        MissingHours, SufficientHours, BestAvailableHours, DailyValidSamples, ...
        DailyStatus, 'VariableNames', {'DateUTC', 'H3_hours', 'H0_hours', ...
        'H1_hours', 'H2_hours', 'Missing_hours', 'Sufficient_hours', ...
        'Best_available_hours', 'N_valid_day', 'Status'});
...
function mask = hoursWithSufficientCandidate(candidatePools, minimumCoverageFraction)
    mask = false(24, 1);
    for productIndex = 1:numel(candidatePools)
        pool = candidatePools{productIndex};
        for candidateIndex = 1:numel(pool)
            result = pool(candidateIndex).Result;
            coverage = double(result.Coverage);
            mask = mask | (result.NValid > 0 & isfinite(coverage) & ...
                coverage >= minimumCoverageFraction);
        end
    end
end


function [pool, logTable] = readCandidates( ...
        dateUTC, product, productCode, candidates, config, logTable)

    pool = repmat(emptyCandidate(), 0, 1);
    ...
    for i = 1:height(candidates)
        ...
        if candidateResult.Usable
            ...
            selected.ProductCode = uint8(productCode);
            selected.Version = candidate.Version;
            selected.Path = candidate.Path;
            selected.ModifiedDatenum = candidate.ModifiedDatenum;
            selected.Result = candidateResult;
            pool(end + 1, 1) = selected; %#ok<AGROW>
            if all(hoursWithSufficientCandidate({pool}, ...
                    config.MinimumCoverageFraction))
                break;
            end
        else
...
function [selected, selectionModeCode] = selectHourCandidate( ...
        candidatePools, hourIndex, minimumCoverageFraction)

    selected = emptyCandidate();
    selectionModeCode = uint8(0);

    % Primary pass: product priority dominates once coverage is sufficient.
    % Candidate order within one product is newest version first.
    for productIndex = 1:numel(candidatePools)
        pool = candidatePools{productIndex};
        for candidateIndex = 1:numel(pool)
            result = pool(candidateIndex).Result;
            coverage = double(result.Coverage(hourIndex));
            if result.NValid(hourIndex) > 0 && isfinite(coverage) && ...
                    coverage >= minimumCoverageFraction
                selected = pool(candidateIndex);
                selected.Selected = true;
                selectionModeCode = uint8(1);
                return;
            end
        end
    end

    % Secondary pass: choose greatest non-zero coverage. Iterating in product
    % priority/newest-version order and retaining equal values implements the
    % documented tie-break without letting one H3 sample beat fuller H0 data.
    bestCoverage = -Inf;
    tieTolerance = 1e-12;
    for productIndex = 1:numel(candidatePools)
        pool = candidatePools{productIndex};
        for candidateIndex = 1:numel(pool)
            ...
            coverage = double(result.Coverage(hourIndex));
            if result.NValid(hourIndex) > 0 && isfinite(coverage) && ...
                    coverage > bestCoverage + tieTolerance
                selected = pool(candidateIndex);
                bestCoverage = coverage;
            end
        end
    end
    if selected.ProductCode ~= 0
        selected.Selected = true;
        selectionModeCode = uint8(2);
    end
end
...
function [result, status, message] = readMfiDay(filePath, dateUTC, product, config)
    ...
    try
        ...
        expectedPerHour = expectedSamplesPerHour(product);
...
function count = expectedSamplesPerHour(product)
    % Native nominal cadences: H3=1 s, H0=16 s, H1=4 min, H2=1 hour.
    switch upper(string(product))
        case "H3"
            count = 3600;
        case "H0"
            count = 225;
        ...
        case "H2"
            count = 1;
        otherwise
            error('ACE:UnknownProduct', 'Unknown ACE/MFI product: %s', product);
    end
end
...
function selected = emptyCandidate()
    selected = struct( ...
        'Selected', false, ...
        'Product', "", ...
        'ProductCode', uint8(0), ...
...
function tf = checkpointIsValid(C, nHours, nDays)
    ...
    required = { ...
        ...
        'SourceCode', 'SourceVersion', 'SourceFile', 'SelectionModeCode', ...
        'YearLog'};
    ...
    hourlyNumeric = {'BMean', 'BMax', 'NTotal', 'NValid', 'Coverage', ...
                     'SourceCode', 'SourceVersion', 'SelectionModeCode'};
    ...
    if ~isstring(C.SourceFile) || ~iscolumn(C.SourceFile) || ...
            numel(C.SourceFile) ~= nHours
        return;
    end
...
function validateFinalResult(Hourly, DailySelection, startDate, endDate, ...
        expectedRows, minimumCoverageFraction)
    ...
    if any(isundefined(Hourly.Source)) || any(isundefined(Hourly.SelectionMode))
        error('ACE:UndefinedCategory', ...
            'Source and SelectionMode must use only their documented categories.');
    end
    dailyCounts = [ ...
        sum(sourceByDay == "H3", 1).', ...
        sum(sourceByDay == "H0", 1).', ...
        sum(sourceByDay == "H1", 1).', ...
        sum(sourceByDay == "H2", 1).', ...
        sum(sourceByDay == "MISSING", 1).'];
    savedCounts = double([ ...
        DailySelection.H3_hours, DailySelection.H0_hours, ...
        DailySelection.H1_hours, DailySelection.H2_hours, ...
        DailySelection.Missing_hours]);
    if any(dailyCounts ~= savedCounts, 'all')
        error('ACE:DailySourceCountMismatch', ...
            'Daily product-hour counts do not match hourly source selections.');
    end
    if any(sum(savedCounts, 2) ~= 24)
        error('ACE:InvalidDailyHourCount', ...
            'Daily product and missing-hour counts must sum to 24.');
    end

    modeByDay = reshape(string(Hourly.SelectionMode), 24, []);
    savedModeCounts = double([DailySelection.Sufficient_hours, ...
        DailySelection.Best_available_hours]);
    actualModeCounts = [sum(modeByDay == "SUFFICIENT", 1).', ...
        sum(modeByDay == "BEST_AVAILABLE", 1).'];
    if any(savedModeCounts ~= actualModeCounts, 'all')
        error('ACE:DailySelectionModeCountMismatch', ...
            'Daily selection-mode counts do not match hourly records.');
    end

    sourceMissing = string(Hourly.Source) == "MISSING";
    modeMissing = string(Hourly.SelectionMode) == "MISSING";
    if any(sourceMissing ~= modeMissing)
        error('ACE:SelectionModeMismatch', ...
            'Source and SelectionMode missing states are inconsistent.');
    end
    sufficient = string(Hourly.SelectionMode) == "SUFFICIENT";
    bestAvailable = string(Hourly.SelectionMode) == "BEST_AVAILABLE";
    if any(sufficient & (~isfinite(Hourly.Coverage) | ...
            Hourly.Coverage < minimumCoverageFraction))
        error('ACE:InsufficientPrimaryCoverage', ...
            'A SUFFICIENT hourly row is below MinimumCoverageFraction.');
    end
    if any(bestAvailable & (~isfinite(Hourly.Coverage) | ...
            Hourly.Coverage <= 0 | Hourly.Coverage >= minimumCoverageFraction))
        error('ACE:InvalidBestAvailableCoverage', ...
            'A BEST_AVAILABLE row must have non-zero sub-threshold coverage.');
    end
    if any(sourceMissing & ~isnan(Hourly.Coverage))
        error('ACE:UnexpectedMissingCoverage', ...
            'Missing hourly rows must have NaN coverage.');
    end
    if any(~sourceMissing & strlength(Hourly.SourceFile) == 0)
        error('ACE:MissingSourceFile', ...
            'Every selected hourly result must retain its source file path.');
    end
    if any(sourceMissing & strlength(Hourly.SourceFile) ~= 0)
        error('ACE:UnexpectedMissingSourceFile', ...
            'Missing hourly rows must not name a source file.');
    end
    ...
    if any(sourceMissing ~= (Hourly.N_valid == 0))
        error('ACE:SourceAvailabilityMismatch', ...
            'Hourly Source does not match valid-sample availability.');
    end
    if any(Hourly.N_total < Hourly.N_valid)
        error('ACE:TotalBelowValidCount', ...
            'N_total cannot be smaller than N_valid.');
    end
    if any(~sourceMissing & Hourly.SourceVersion == 0) || ...
            any(sourceMissing & Hourly.SourceVersion ~= 0)
        error('ACE:SourceVersionMismatch', ...
            'SourceVersion must be positive only for selected hourly rows.');
    end
    dailyValid = uint32(sum(reshape(double(Hourly.N_valid), 24, []), 1).');
    if any(dailyValid ~= DailySelection.N_valid_day)
        error('ACE:DailyValidCountMismatch', ...
            'Daily N_valid totals do not match the hourly records.');
    end
