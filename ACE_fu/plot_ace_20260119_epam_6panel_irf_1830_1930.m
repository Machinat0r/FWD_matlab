%% ACE 2026-01-19 18:30-19:30 UTC magnetic field and EPAM plots
...
fprintf('\nValid EPAM samples in the loaded cache (display: 18:30-19:30):\n');
...
%% Local robust color limits for the displayed one-hour interval
epmInPlot = tEpm >= plot0 & tEpm <= plot1;
specFluxGroups = {J_LEMS30, J_LEMS120, J_DE, J_LEFS60, J_LEFS150};
specClim = nan(numel(specFluxGroups), 2);
for iiSpec = 1:numel(specFluxGroups)
    localLog = log10(specFluxGroups{iiSpec}(epmInPlot, :));
    localLog = sort(localLog(isfinite(localLog)));
    if isempty(localLog)
        specClim(iiSpec, :) = [0 1];
        continue
    end
    nLocal = numel(localLog);
    lo = localLog(max(1, round(0.10 .* nLocal)));
    hi = localLog(min(nLocal, round(0.98 .* nLocal)));
    if hi <= lo
        lo = lo - 0.5;
        hi = hi + 0.5;
    end
    specClim(iiSpec, :) = [lo hi];
end

%% Six IRF panels
h = irf_plot(6, 'newfigure');
fig = gcf;
set(fig, 'Color', 'w', 'Name', 'ACE 2026-01-19 18:30-19:30 EPAM six-panel', ...
...
for iiSpec = 1:size(specClim, 1)
    clim(h(iiSpec + 1), specClim(iiSpec, :));
end
...
set(h(6), 'XTickLabel', timeTickLabels);
xlabel(h(6), '2026-01-19 UTC');
...
outFig = fullfile(baseDir, 'ace_20260119_1830_1930_epam_6panel_irf.fig');
outPng = fullfile(baseDir, 'ace_20260119_1830_1930_epam_6panel_irf.png');
outPdf = fullfile(baseDir, 'ace_20260119_1830_1930_epam_6panel_irf.pdf');
...
figLine = gcf;
set(figLine, 'Color', 'w', 'Name', 'ACE 2026-01-19 18:30-19:30 EPAM line plots', ...
...
%% Line-panel labels and ordered physical energy legends
lineLetters = 'abcdef';
...
lems30Lines = flipud(findobj(hLine(2), 'Type', 'line'));
lems30Legend = legend(hLine(2), lems30Lines, ...
    {'P_7: 1.06-1.88 MeV','P_8: 1.88-4.70 MeV'}, ...
    'Location', 'northeast', 'NumColumns', 2, 'Box', 'off', ...
    'FontSize', 7.5, 'Interpreter', 'tex', 'AutoUpdate', 'off');
lems30Legend.ItemTokenSize = [10 7];
...
lems120Lines = flipud(findobj(hLine(3), 'Type', 'line'));
lems120Legend = legend(hLine(3), lems120Lines, lems120Labels, ...
    'Location', 'northeast', 'NumColumns', 2, 'Box', 'off', ...
    'FontSize', 6.5, 'Interpreter', 'tex', 'AutoUpdate', 'off');
lems120Legend.ItemTokenSize = [9 7];
...
deLines = flipud(findobj(hLine(4), 'Type', 'line'));
deLegend = legend(hLine(4), deLines, deLabels, ...
    'Location', 'northeast', 'NumColumns', 2, 'Box', 'off', ...
    'FontSize', 7, 'Interpreter', 'tex', 'AutoUpdate', 'off');
deLegend.ItemTokenSize = [10 7];
...
lefs60Lines = flipud(findobj(hLine(5), 'Type', 'line'));
lefs60Legend = legend(hLine(5), lefs60Lines, lefs60Labels, ...
    'Location', 'northeast', 'NumColumns', 2, 'Box', 'off', ...
    'FontSize', 7, 'Interpreter', 'tex', 'AutoUpdate', 'off');
lefs60Legend.ItemTokenSize = [10 7];

text(hLine(6), 0.10, 0.88, 'LEFS150: e^- (+ions)', 'Units', 'normalized', ...
    'FontSize', 9, 'FontWeight', 'bold', 'Color', 'k', 'Interpreter', 'tex');
text(hLine(6), 0.78, 0.70, 'E_{1-3}: fill in 2026', 'Units', 'normalized', ...
    'FontSize', 7, 'Color', [0.35 0.35 0.35], 'Interpreter', 'tex');
lefs150Lines = flipud(findobj(hLine(6), 'Type', 'line'));
lefs150Legend = legend(hLine(6), lefs150Lines, {'E_4: 180-295 keV'}, ...
    'Location', 'northeast', 'Box', 'off', 'FontSize', 8, ...
    'Interpreter', 'tex', 'AutoUpdate', 'off');
lefs150Legend.ItemTokenSize = [10 7];
...
for ii = 1:numel(hLine)
    set(hLine(ii), 'XLim', [timeTicks(1) timeTicks(end)], ...
        'XTick', timeTicks, 'XMinorTick', 'off');
end
for ii = 1:5
    set(hLine(ii), 'XTickLabel', []);
end
set(hLine(6), 'XTickLabel', timeTickLabels);
xlabel(hLine(6), '2026-01-19 UTC');
...
outLineFig = fullfile(baseDir, 'ace_20260119_1830_1930_epam_lineplots_irf.fig');
outLinePng = fullfile(baseDir, 'ace_20260119_1830_1930_epam_lineplots_irf.png');
outLinePdf = fullfile(baseDir, 'ace_20260119_1830_1930_epam_lineplots_irf.pdf');

set(figLine, 'PaperPositionMode', 'auto');
savefig(figLine, outLineFig);
exportgraphics(figLine, outLinePng, 'Resolution', 300, 'BackgroundColor', 'white');
exportgraphics(figLine, outLinePdf, 'ContentType', 'vector', 'BackgroundColor', 'white');
