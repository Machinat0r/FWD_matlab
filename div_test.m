clear;clc;close all
%------written by Wending Fu, Feb.19.2022 in Beijing------------
%The program simulates a 3d vector field generated by a point charge
%There is a source/sink zero near the space zero point
...
x = -50:0.5:50;x=x';
units = irf_units;
% k0 = 1/(4*pi*units.eps0);
k0 = 1/(4*pi);
Q = 1e5;
r0 = [0,0,0];
rr = 0;

% a1 = [0,0,3]; a2 = [-2*sqrt(2),0,-3]; a3 = [sqrt(2),sqrt(6),-1]; a4 = [sqrt(2),-sqrt(6),-1];
a1 = 10*[0,0,3]; a2 = 10*[-2*sqrt(2)-1,2,-1]; a3 = 10*[sqrt(2)+2,sqrt(6)+1,-2]; a4 = 10*[sqrt(2)-1,-sqrt(6)+1,-3];
% a1 = [0,0,3]; a2 = [0,-2*sqrt(2),-1]; a3 = [sqrt(6),sqrt(2),-1]; a4 = [-sqrt(6),sqrt(2),-1];
% c_eval('a? = a? * 2;');
...
swap_var(a2,a4);
%% 
c_eval('r? = sqrt((a?(:,2)-r0(1)).^2+(a?(:,3)-r0(2)).^2+(a?(:,4)-r0(3)).^2)-rr;',1:4);
c_eval("B? = transpose(k0*Q./(r?'.^2));",1:4);
c_eval('B?(r?<=0,1)=0;',1:4);
c_eval('B?(:,2) = B?(:,1).*(a?(:,2)./r?);',1:4);
c_eval('B?(:,3) = B?(:,1).*(a?(:,3)./r?);',1:4);
c_eval('B?(:,4) = B?(:,1).*(a?(:,4)./r?);',1:4);

% [Br1,Br2,Br3,Br4] =  EliminateCurl('a?','E?(:,2:4)',1:length(B1));
% [Loc,Q] = solveMonopole('a?','Br?',1213)
%% PI
...
% % % h(i)=subplot(n,1,i);
% % % 
% % % plot(temp,transpose(mean(dLoc,2)), 'color','k', 'Linewidth',0.75); hold on;
% % % grid off;
...
%% Q Error
% % % h(i)=subplot(n,1,i);
% % % plot(temp,Qerror, 'color','k', 'Linewidth',0.75); hold on;
% % % grid off;
